...
playlists.post('/', async (c) => {
    ...
    try {
        ...
        if (!name || !videoLimit) {
            return c.json({error: 'Name, video limit, does owner vote count are required'}, 400)
        } else if (doesOwnerVoteCount != 0 && doesOwnerVoteCount != 1) {
            return c.json({error: 'Fix your vars bruh'}, 400)
        }

        return new Promise((resolve) => {
            db.run('INSERT INTO playlists (name, videoLimit, doesOwnerVoteCount, userId) VALUES (?, ?, ?, ?)',
                [name, videoLimit, doesOwnerVoteCount, user.id], function (err) {
                    if (err) {
                        resolve(c.json({error: 'Database error'}, 500))
                    } else {
                        resolve(c.json({success: true, message: 'Playlist created successfully'}, 201))
                    }
                })
        })
    } catch (error) {
